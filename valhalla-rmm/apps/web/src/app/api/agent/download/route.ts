...
//
// Serves pre-configured agent scripts with the org API key already embedded.
// Requires the user to be authenticated — prevents key leakage to unauthenticated requests.
...
export async function GET(req: NextRequest) {
  ...
  const platform     = searchParams.get('platform') || 'windows'   // windows | mac | linux
  ...
  const endpoint = `https://${process.env.NEXT_PUBLIC_SUPABASE_URL?.replace('https://', '')}/functions/v1/register-device`
...
<#
.SYNOPSIS
    Valhalla IT Asset Agent — Windows
    Pre-configured for your organization. Run as Administrator to install.

.DESCRIPTION
    Run without parameters to check in once.
    Run with -Install to set up daily scheduled task (requires Administrator).
    Run with -Uninstall to remove the scheduled task.
#>
...
# ── Pre-configured values (do not edit) ──────────────────────────────────────
$ApiKey        = "${apiKey}"
...
if ($Install) {
    Write-Log "Installing Valhalla IT Agent as daily scheduled task..."
    if (-not (Test-Path $AgentDir)) { New-Item -ItemType Directory -Path $AgentDir -Force | Out-Null }
    ...
    $trigger  = New-ScheduledTaskTrigger -Daily -At $TaskSchedule
    ...
    try {
        Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger \`
            ...
            -Description "Valhalla IT Asset Agent — daily device check-in" -Force | Out-Null
        Write-Log "Scheduled task created. Runs daily at $TaskSchedule."
    } catch {
        Write-Log "Failed to create scheduled task: $_" "ERROR"
        Write-Log "Make sure you are running this script as Administrator." "WARN"
        exit 1
    }
...
# ── Collect system info and check in ─────────────────────────────────────────
...
try {
    ...
    Write-Log "Failed to collect system info: $_" "ERROR"; exit 1
}
...
try {
    ...
    Write-Log "Check-in failed: $_" "ERROR"; exit 1
}
...
`
  ...
  // ── Generate macOS / Linux bash script ──────────────────────────────────────
  const script = `#!/bin/bash
# Valhalla IT Asset Agent — macOS / Linux
...
log() {
    local msg="[$(date '+%Y-%m-%d %H:%M:%S')] [${2:-INFO}] $1"
    echo "$msg"
    mkdir -p "$AGENT_DIR"
    echo "$msg" >> "$LOG_FILE"
}
...
if [[ "$OS_TYPE" == "macos" ]]; then
    ...
    DISK_TOTAL_KB="$(echo "$DISK_INFO" | awk '{print $2}')"; DISK_FREE_KB="$(echo "$DISK_INFO" | awk '{print $4}')"
    DISK_GB="$(echo "scale=1; $DISK_TOTAL_KB / 1048576" | bc 2>/dev/null)"
    DISK_FREE_GB="$(echo "scale=1; $DISK_FREE_KB / 1048576" | bc 2>/dev/null)"
...
RESPONSE="$(curl -s -w "\\n%{http_code}" -X POST "$AGENT_ENDPOINT" \\
    ...
    -d "{\\\"api_key\\\":\\\"$API_KEY\\\",\\\"hostname\\\":\\\"$HOSTNAME_VAL\\\",\\\"os\\\":\\\"$OS_NAME\\\",\\\"os_version\\\":\\\"$OS_VERSION\\\",\\\"os_type\\\":\\\"$OS_TYPE\\\",\\\"cpu\\\":\\\"$CPU\\\",\\\"ram_gb\\\":${RAM_GB:-null},\\\"disk_gb\\\":${DISK_GB:-null},\\\"disk_free_gb\\\":${DISK_FREE_GB:-null},\\\"ip_address\\\":\\\"$IP_ADDRESS\\\",\\\"mac_address\\\":\\\"$MAC_ADDRESS\\\",\\\"manufacturer\\\":\\\"$MANUFACTURER\\\",\\\"model\\\":\\\"$MODEL\\\",\\\"serial_number\\\":\\\"$SERIAL\\\",\\\"agent_version\\\":\\\"$AGENT_VERSION\\\",\\\"customer_name\\\":\\\"$CUSTOMER_NAME\\\"}" \\
    --max-time 30 2>&1)"
