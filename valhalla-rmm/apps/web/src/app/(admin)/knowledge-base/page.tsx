  ...
  useEffect(() => {
    if (editing) {
      setForm({
        ...
        category:     editing.category     || 'how_to',
      ...
      <div className="flex items-start justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-800">
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${CATEGORY_CLS[article.category] ?? ''}`}>
              {lbl(article.category)}
            </span>
      ...
      <div className="px-5 py-5">
        ...
        <div className="mt-8 pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center gap-4">
          ...
          <button onClick={() => onHelpful(article.id, 'helpful_count', article.helpful_count || 0)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-emerald-300 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-700 dark:text-emerald-400 dark:hover:bg-emerald-950/30 text-sm transition-colors">
            👍 Helpful ({article.helpful_count || 0})
          </button>
          <button onClick={() => onHelpful(article.id, 'view_count', article.view_count || 0)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-rose-300 text-rose-700 hover:bg-rose-50 dark:border-rose-700 dark:text-rose-400 dark:hover:bg-rose-950/30 text-sm transition-colors">
            👎 Not Helpful
          </button>
        </div>
      </div>
...
export default function KnowledgeBasePage() {
  ...
  const loadArticles = async () => {
    ...
    const { data } = await supabase
      .from('knowledge_articles').select('*').order('updated_at', { ascending: false }).limit(200)
    setArticles(data ?? [])
    setLoading(false)
  }
      ...
      <div className="flex flex-wrap gap-2 items-center justify-between">
        <div className="flex flex-wrap gap-2 items-center flex-1">
          ...
          <button onClick={() => setShowDrafts(!showDrafts)}
            className={`text-xs px-3 py-2 rounded-lg border transition-colors ${showDrafts ? 'bg-amber-100 text-amber-700 border-amber-300 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-700' : 'text-slate-500 border-slate-200 dark:border-slate-700 hover:border-amber-400'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {loading ? (
            ...
            <div key={article.id}
              ...
              className={`bg-white dark:bg-slate-900 rounded-xl border shadow-sm hover:shadow-md transition-shadow cursor-pointer group p-5 ${
                !article.is_published
                  ? 'opacity-75 border-dashed border-amber-300 dark:border-amber-700'
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                {(article.helpful_count > 0) && (
                  <p className="text-xs text-slate-400">👍 {article.helpful_count}</p>
                )}
              </div>
            </div>
          ))}
