...
import {
  ...
  Clock, Ticket, FileText, Plus, Edit, Trash2, User, X,
} from 'lucide-react'
...
export default function CustomerDetailPage() {
  ...
  const loadAll = async () => {
    ...
    setCustomer(cust.data); setTickets(tick.data ?? []); setTimeEntries(time.data ?? [])
    setInvoices(inv.data ?? []); setContacts(cont.data ?? [])
    setLoading(false)
  }
  ...
  const openEdit = () => {
    if (!customer) return
    setForm({
      name: customer.name ?? '', status: customer.status ?? 'active',
      contract_type: customer.contract_type ?? '', industry: customer.industry ?? '',
      contact_name: customer.contact_name ?? '', contact_email: customer.contact_email ?? '',
      contact_phone: customer.contact_phone ?? '', monthly_rate: customer.monthly_rate ?? '',
      hourly_rate: customer.hourly_rate ?? '', after_hours_rate: customer.after_hours_rate ?? '',
  ...
  const saveCustomer = async (e) => {
    ...
    const { error } = await supabase.from('customers').update({
      name: form.name, status: form.status, contract_type: form.contract_type || null,
      industry: form.industry || null, contact_name: form.contact_name || null,
  ...
  const setupSubscription = async () => {
    if (!customer.monthly_rate) { alert('Please set a monthly rate on this customer first.'); return }
    setStripeLoading(true)
    const { data, error } = await supabase.functions.invoke('stripe-create-subscription', {
      body: {
        customer_id:  id,
        monthly_rate: customer.monthly_rate,
        description:  `Managed Services — ${customer.name}`,
      }
    })
  ...
  return (
    <div className="max-w-5xl space-y-4">
      ...
      {/* Stripe subscription card — only for managed clients */}
      {customer.contract_type === 'managed' && (
        <div className={`rounded-xl border p-4 ${customer.stripe_subscription_id ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/40' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-white">
                {customer.stripe_subscription_id ? '✓ Recurring billing active' : 'Set up recurring billing'}
              </p>
              <p className="text-xs text-slate-500 mt-0.5">
                {customer.stripe_subscription_id
                  ? `Auto-charges $${customer.monthly_rate?.toLocaleString()}/mo via Stripe`
                  : customer.monthly_rate
                    ? `Auto-charge $${customer.monthly_rate?.toLocaleString()}/mo — client enters card once`
                    : 'Set a monthly rate on this customer to enable Stripe billing'}
              </p>
            </div>
            {!customer.stripe_subscription_id && customer.monthly_rate && (
              <button onClick={setupSubscription} disabled={stripeLoading}
                className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white rounded-lg text-sm font-semibold transition-colors flex-shrink-0">
                {stripeLoading ? 'Loading…' : 'Set Up Billing'}
              </button>
            )}
            {customer.stripe_subscription_id && (
              <span className="text-xs text-emerald-600 font-medium flex-shrink-0">Active</span>
            )}
          </div>
        </div>
      )}
      ...
      {activeTab === 'contacts' && (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {contacts.length === 0 ? (
              <p className="px-4 py-10 text-center text-slate-400 text-sm">No contacts yet.</p>
            ) : contacts.map(c => (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={form.contract_type} onChange={e => sf('contract_type', e.target.value)} className={`mt-1 ${inp}`}>
                    <option value="managed">Managed</option><option value="time_and_materials">T&M</option>
                    <option value="block_hours">Block Hours</option><option value="project">Project</option>
                  </select>
                </div>
              </div>
              <div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button onClick={saveContact} disabled={!cForm.name.trim() || cSaving}
                  className="flex-1 px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white rounded-lg text-sm font-semibold">
