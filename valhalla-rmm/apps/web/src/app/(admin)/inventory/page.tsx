...
import {
  ...
  Edit, Trash2, Loader2, Upload, Download, X, Terminal, ExternalLink,
} from 'lucide-react'
  ...
  return (
    <div className="flex flex-col gap-1">
      {tickets.slice(0, 2).map(t => (
        <button key={t.id} onClick={e => { e.stopPropagation(); router.push(`/tickets/${t.id}`) }}
      ...
      {tickets.length > 2 && <span className="text-[11px] text-slate-400">+{tickets.length - 2} more</span>}
    </div>
  )
}
...
const BLANK = { name:'', description:'', category:'hardware', status:'in_stock', quantity:'1', unit_cost:'', vendor:'', model:'', serial_number:'', asset_tag:'', customer_id:'', customer_name:'', location:'', purchase_date:'', warranty_expiry:'', notes:'' }
...
function normaliseItarianStatus() { return 'deployed' }
...
function CsvImportDialog({ open, onClose, onImported, orgId, customers }) {
  ...
  const handleFile = (file) => {
    ...
    reader.onload = (e) => {
      ...
      if (isItarianCsv(hdrs)) {
        const itarianMap = { 'Name':'name','Serial Number':'serial_number','Device status':'status','Customer':'customer_name','OS name':'category','Model':'model','System Manufacturer':'vendor','Internal IP':'ip_address','External IP':'external_ip','Last Activity':'last_seen','OS version':'os','Device Group':'location','Registered':'purchase_date','Notes':'notes' }
        hdrs.forEach(h => { if (itarianMap[h]) autoMap[itarianMap[h]] = h })
        autoMap._itarian = true
      } else {
        hdrs.forEach(h => {
  ...
  useEffect(() => {
    ...
    rows.slice(0,3).forEach((row, i) => {
      ...
      prev.push({ name: name || '(missing)', category: mapping.category ? normaliseCategory(row[mapping.category]) : 'hardware', status: mapping.status ? normaliseStatus(row[mapping.status]) : 'in_stock', serial_number: mapping.serial_number ? row[mapping.serial_number] : '' })
    })
  ...
  const handleImport = async () => {
    ...
    for (const row of rows) {
      ...
      let lastSeenAt = null
      if (lastSeen) { try { lastSeenAt = new Date(lastSeen).toISOString() } catch {} }
      ...
      if (isItarian && mapping.category) {
        ...
        if (os.includes('windows')||os.includes('macos')||os.includes('linux')) category = 'hardware'
        else if (os.includes('ios')||os.includes('android')) category = 'other'
      } else if (mapping.category) { category = normaliseCategory(row[mapping.category]) }
      const payload = {
        organization_id: orgId, name, category,
        ...
        online: isOnline, source: isItarian ? 'xcitium' : 'manual',
      }
      if (serial) {
        const { data: existing } = await supabase.from('inventory_items').select('id').eq('organization_id', orgId).eq('serial_number', serial).single()
        if (existing) await supabase.from('inventory_items').update(payload).eq('id', existing.id)
        else await supabase.from('inventory_items').insert(payload)
      } else { await supabase.from('inventory_items').insert(payload) }
      count++
    }
    setImported(count); setDone(true); setImporting(false); onImported()
  }
        ...
        <div className="p-5 space-y-4">
          {done ? (
            <div className="text-center py-8">
              <div className="w-14 h-14 rounded-full bg-emerald-100 flex items-center justify-center mx-auto mb-3"><Package className="w-7 h-7 text-emerald-600" /></div>
            ...
            <>
              <div className="flex items-center justify-between">
                ...
                <button onClick={downloadTemplate} className="flex items-center gap-1.5 text-xs text-amber-600 hover:text-amber-700 font-medium"><Download className="w-3.5 h-3.5" /> Download Template</button>
              </div>
              {!rows.length ? (
                <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-10 text-center cursor-pointer hover:border-amber-400 transition-colors"
                ...
                <>
                  ...
                  {errors.length > 0 && <div className="bg-rose-50 border border-rose-200 rounded-lg p-3 space-y-1">{errors.map((e, i) => <p key={i} className="text-xs text-rose-700">{e}</p>)}</div>}
                  {preview.length > 0 && (
                    <div>
...
function ItemDialog({ open, onClose, onSaved, editing, orgId, customers }) {
  ...
  useEffect(() => {
    if (!open) return
    if (editing) {
      setForm({ name: editing.name||'', description: editing.description||'', category: editing.category||'hardware', status: editing.status||'in_stock', quantity: String(editing.quantity??1), unit_cost: editing.unit_cost!=null?String(editing.unit_cost):'', vendor: editing.vendor||'', model: editing.model||'', serial_number: editing.serial_number||'', asset_tag: editing.asset_tag||'', customer_id: editing.customer_id||'', customer_name: editing.customer_name||'', location: editing.location||'', purchase_date: editing.purchase_date||'', warranty_expiry: editing.warranty_expiry||'', notes: editing.notes||'' })
    } else { setForm({ ...BLANK }) }
  }, [editing, open])
  const handleSave = async () => {
    ...
    const payload = { organization_id: orgId, name: form.name.trim(), description: form.description||null, category: form.category, status: form.status, quantity: parseInt(form.quantity)||1, unit_cost: form.unit_cost!==''?parseFloat(form.unit_cost):null, vendor: form.vendor||null, model: form.model||null, serial_number: form.serial_number||null, asset_tag: form.asset_tag||null, customer_id: form.customer_id||null, customer_name: form.customer_name||null, location: form.location||null, purchase_date: form.purchase_date||null, warranty_expiry: form.warranty_expiry||null, notes: form.notes||null }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-800">
          <h2 className="font-semibold text-slate-900 dark:text-white">{editing ? 'Edit Item' : 'Add Inventory Item'}</h2>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Name *</label><input value={form.name} onChange={e => s('name', e.target.value)} placeholder="e.g., Dell Latitude 5540 Laptop" className={`mt-1 ${inp}`} /></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Category</label><select value={form.category} onChange={e => s('category', e.target.value)} className={`mt-1 ${inp}`}>{CATEGORIES.map(c => <option key={c} value={c}>{lbl(c)}</option>)}</select></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Status</label><select value={form.status} onChange={e => s('status', e.target.value)} className={`mt-1 ${inp}`}>{STATUSES.map(st => <option key={st} value={st}>{lbl(st)}</option>)}</select></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Vendor / Manufacturer</label><input value={form.vendor} onChange={e => s('vendor', e.target.value)} className={`mt-1 ${inp}`} /></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Model</label><input value={form.model} onChange={e => s('model', e.target.value)} className={`mt-1 ${inp}`} /></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Serial Number</label><input value={form.serial_number} onChange={e => s('serial_number', e.target.value)} className={`mt-1 ${inp} font-mono`} /></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Asset Tag</label><input value={form.asset_tag} onChange={e => s('asset_tag', e.target.value)} className={`mt-1 ${inp}`} /></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Quantity</label><input type="number" min={1} value={form.quantity} onChange={e => s('quantity', e.target.value)} className={`mt-1 ${inp}`} /></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Unit Cost ($)</label><input type="number" min={0} step="0.01" value={form.unit_cost} onChange={e => s('unit_cost', e.target.value)} className={`mt-1 ${inp}`} /></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Assigned Customer</label>
              <select value={form.customer_id||'__none__'} onChange={e => { const c = customers.find(x => x.id===e.target.value); s('customer_id', e.target.value==='__none__'?'':e.target.value); s('customer_name', c?.name||'') }} className={`mt-1 ${inp}`}>
                <option value="__none__">Not Assigned</option>{customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Location</label><input value={form.location} onChange={e => s('location', e.target.value)} className={`mt-1 ${inp}`} /></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Purchase Date</label><input type="date" value={form.purchase_date} onChange={e => s('purchase_date', e.target.value)} className={`mt-1 ${inp}`} /></div>
            <div><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Warranty Expiry</label><input type="date" value={form.warranty_expiry} onChange={e => s('warranty_expiry', e.target.value)} className={`mt-1 ${inp}`} /></div>
            <div className="col-span-2"><label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Notes</label><textarea value={form.notes} onChange={e => s('notes', e.target.value)} rows={2} className={`mt-1 ${inp} resize-none`} /></div>
          </div>
          {editing && (editing.last_seen_at||editing.ip_address||editing.cpu) && (
            <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Agent Data <span className="font-normal text-slate-400 normal-case">(auto-collected)</span></p>
...
export default function InventoryPage() {
  ...
  const expiringCount = items.filter(i => warrantyInfo(i.warranty_expiry)?.expiring).length
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 flex items-center gap-3">
            ...
            <div><p className={`text-xl font-bold ${s.color}`}>{s.value}</p><p className="text-xs text-slate-400">{s.label}</p></div>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-2 items-center justify-between">
        <div className="flex flex-wrap gap-2 items-center flex-1">
          ...
          <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className={sel}>
            <option value="all">All Categories</option>{CATEGORIES.map(c => <option key={c} value={c}>{lbl(c)}</option>)}
          </select>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className={sel}>
            <option value="all">All Status</option>{STATUSES.map(st => <option key={st} value={st}>{lbl(st)}</option>)}
          </select>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                const hasAgent = item.source === 'agent' || !!item.hostname
                return (
                  <tr key={item.id}
                    onClick={() => router.push(`/inventory/${item.id}`)}
                    className="hover:bg-amber-50/30 dark:hover:bg-slate-800/30 transition-colors cursor-pointer">
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-2">
                        <div>
                          <p className="font-medium text-slate-900 dark:text-white group-hover:text-amber-600">{item.name}</p>
                          ...
                          {item.last_seen_at ? (() => {
                            ...
                            return <p className={`text-[11px] ${color} flex items-center gap-1 mt-0.5`}><span className={`w-1.5 h-1.5 rounded-full ${dot} inline-block`} />{days>7?`Offline · ${ago}`:days>1?`Stale · ${ago}`:`Last seen ${ago}`}</p>
                          })() : hasAgent ? null : (
                            <p className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5"><span className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-600 inline-block" />No agent</p>
                          )}
                        </div>
                        {hasAgent && <ExternalLink className="w-3 h-3 text-slate-300 dark:text-slate-600 flex-shrink-0" />}
                      </div>
                    </td>
                    <td className="px-3 py-3"><span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${CAT_CLS[item.category]??''}`}>{item.category}</span></td>
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3"><span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${STATUS_CLS[item.status]??''}`}>{lbl(item.status)}</span></td>
                    ...
                    <td className="px-3 py-3" onClick={e => e.stopPropagation()}><LinkedTicketsBadge assetId={item.id} /></td>
                    <td className="px-3 py-3" onClick={e => e.stopPropagation()}>
                      <div className="flex gap-1">
                        <button onClick={e => { e.stopPropagation(); setEditing(item); setDialogOpen(true) }}
        ...
        {filtered.length > 0 && (
          <div className="px-4 py-2.5 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs text-slate-400">
            <span>{filtered.length} items — click any row to open device details</span>
      ...
      <ItemDialog open={dialogOpen} onClose={() => { setDialogOpen(false); setEditing(null) }} onSaved={() => { setDialogOpen(false); setEditing(null); loadAll() }} editing={editing} orgId={orgId} customers={customers} />
      <CsvImportDialog open={importOpen} onClose={() => setImportOpen(false)} onImported={() => { setImportOpen(false); loadAll() }} orgId={orgId} customers={customers} />
    </div>
  )
}
