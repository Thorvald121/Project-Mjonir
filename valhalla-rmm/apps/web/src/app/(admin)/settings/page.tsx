...
import {
  ...
  AlertCircle, Settings2, Mail, Users, Lock,
} from 'lucide-react'
  ...
  return (
    <Section title="Organization" description="Core company info used across the platform.">
      ...
      <FieldRow label="Support Email" hint="Must match the Gmail account connected for email-to-ticket ingestion.">
        <input type="email" value={form.company_email} onChange={e => sf('company_email', e.target.value)}
          placeholder="support@yourcompany.com" className={inp} />
      </FieldRow>
      <FieldRow label="App URL" hint="Public URL — used in CSAT links and email footers.">
        <input value={form.app_url} onChange={e => sf('app_url', e.target.value)}
          placeholder="https://valhalla-rmm.com" className={inp} />
      </FieldRow>
      <FieldRow label="Client Portal URL" hint="Share with clients — they land on their support portal after login.">
...
function SlaSection({ org, onSaved }) {
  ...
  useEffect(() => {
    ...
    if (org?.notification_config) {
      try { setNotifications(typeof org.notification_config === 'string' ? JSON.parse(org.notification_config) : org.notification_config) } catch {}
    }
  }, [org])
  ...
  const saveNotif = async () => {
    setSavingNotif(true)
    await supabase.from('organizations').update({ notification_config: notifications }).eq('id', org.id)
  ...
  const PRIORITY_COLORS = {
    critical: 'text-rose-500', high: 'text-orange-500', medium: 'text-amber-500', low: 'text-emerald-500',
  }

  return (
    <Section title="SLA & Alerts" description="Set response time targets per priority and configure notification preferences.">
      <div>
        <p className="text-sm font-medium text-slate-900 dark:text-white mb-3">SLA Response Targets (hours)</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {['critical','high','medium','low'].map(p => (
            <div key={p} className="p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
              <p className={`text-xs font-semibold uppercase tracking-wide mb-2 ${PRIORITY_COLORS[p]}`}>{p}</p>
              <div className="flex items-center gap-1.5">
                <input type="number" min={1} value={sla[p] || ''} onChange={e => setSla(p2 => ({ ...p2, [p]: Number(e.target.value) }))}
          ...
          {[
            ...
            <div key={key} className="flex items-start justify-between gap-4 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors flex-shrink-0 ${notifications[key] ? 'bg-amber-500' : 'bg-slate-200 dark:bg-slate-700'}`}>
                <span className={`inline-block h-4 w-4 rounded-full bg-white shadow transition-transform ${notifications[key] ? 'translate-x-6' : 'translate-x-1'}`} />
              </button>
            </div>
          ))}
        </div>
...
function IntegrationsSection({ org }) {
  const items = [
    {
      name: 'Gmail',
      icon: Mail,
      description: 'Inbound email → automatic ticket creation. Client replies sync back as ticket comments.',
      hint: 'Connect via Supabase Edge Functions. The Gmail account must match your Support Email in Organization settings.',
    ...
    },
    {
      ...
      status: 'manual',
      statusLabel: 'Via Supabase secrets',
    },
    {
      name: 'Resend',
      icon: Mail,
      description: 'Transactional email delivery for invoice emails and ticket notifications.',
      hint: 'RESEND_API_KEY is configured in Supabase Edge Function Secrets.',
      status: 'configured',
      statusLabel: 'Configured via secrets',
    },
    {
      name: 'Anthropic AI',
      icon: Zap,
      description: 'AI-powered ticket triage — auto-assigns priority and category on ticket creation.',
  ...
  const statusCls = {
    configured:     'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400',
    not_configured: 'bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400',
          ...
          <div key={item.name} className="flex items-start gap-4 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 ${statusCls[item.status]}`}>
  ...
  return (
    <Section title="Team Members" description="Manage who has access to your Valhalla RMM account.">
      ...
      <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 space-y-3">
        <p className="text-sm font-medium text-slate-900 dark:text-white">Add Team Member</p>
        <div className="flex gap-2">
          ...
          <button onClick={inviteMember} disabled={!invEmail.trim() || inviting}
            className="px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white rounded-lg text-sm font-semibold transition-colors">
...
function AccountSection({ user, onSignOut }) {
  ...
  return (
    <Section title="My Account" description="Your personal account details.">
      <FieldRow label="Email" hint="Email cannot be changed here. Contact your Supabase admin.">
      ...
      <div className="border-t border-slate-200 dark:border-slate-700 pt-5 space-y-3">
        <p className="text-sm font-medium text-slate-900 dark:text-white">Session</p>
