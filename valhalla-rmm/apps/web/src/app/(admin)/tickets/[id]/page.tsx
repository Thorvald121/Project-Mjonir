...
function getSlaInfo(sla_due_date, status) {
...
function NewTicketDialog({ open, onClose, onSaved, customers, orgId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!orgId) { setErr('Organization not found — please sign out and back in'); return }
...
export default function TicketsPage() {
  ...
  const { data: tickets = [], isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('tickets').select('*').order('created_at', { ascending: false }).limit(200)
  ...
  const { data: customers = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from('customers').select('id,name').eq('status','active').order('name').limit(200)
      return data ?? []
    },
  })

  const uniqueCustomers = useMemo(() =>
    [...new Set(tickets.map(t => t.customer_name).filter(Boolean))].sort()
  , [tickets])
        ...
        if (status === 'in_progress' && ticket?.status !== 'in_progress') {
          ...
          if (mins > 0 && orgId) {
            await supabase.from('time_entries').insert({
              ...
              description: `Time on: ${ticket.title}`, minutes: mins,
              billable: true, hourly_rate: 0, date: new Date().toISOString().split('T')[0],
            })
          }
        }
  ...
  return (
    <div className="space-y-4">
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 px-4 py-2.5 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 rounded-lg">
          ...
          <div className="flex items-center gap-2 ml-2">
            ...
            <button disabled={!bulkStatus || bulkMutation.isPending} onClick={() => bulkMutation.mutate(bulkStatus)}
              className="h-7 px-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded text-xs font-medium transition-colors">Apply</button>
          </div>
          ...
          <button onClick={() => setSelected(new Set())} className="h-7 w-7 flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {isError ? (
                <tr><td colSpan={9} className="px-4 py-12 text-center text-rose-500 text-sm">Failed to load tickets. Check your connection and refresh.</td></tr>
              ) : isLoading ? (
                ...
                return (
                  <tr key={t.id} className={`hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors ${selected.has(t.id) ? 'bg-blue-50/50 dark:bg-blue-950/20' : ''}`}>
                    ...
                    <td className="px-3 py-3">
                      {t.sla_due_date ? (
                        <div className={`flex items-center gap-1 text-xs font-medium whitespace-nowrap ${sla.state==='breached'?'text-rose-600':sla.state==='warning'?'text-amber-600':'text-slate-400'}`}>
                          {sla.state==='breached' ? <AlertTriangle className="w-3 h-3"/> : <Clock className="w-3 h-3"/>}
                          {sla.label}
                        </div>
                      ) : <span className="text-slate-300 text-xs">—</span>}
                    </td>
                    <td className="px-3 py-3 text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">{formatDate(t.created_at)}</td>
                  </tr>
                )
              })}
            </tbody>
      ...
      <NewTicketDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSaved={() => qc.invalidateQueries({ queryKey: ['tickets'] })} customers={customers} orgId={orgId} />
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <p className="text-sm text-slate-500 mb-6">This cannot be undone. All associated data will be permanently removed.</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDelete(false)} className="flex-1 px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">Cancel</button>
              <button onClick={() => { deleteMutation.mutate(selected); setConfirmDelete(false) }} className="flex-1 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-sm font-semibold transition-colors">
