...
import type { Ticket, TicketComment } from '@valhalla/types'
import {
...
export default function TicketDetailClient() {
  ...
  const { data: techUsers = [] } = useQuery<{ id: string; user_email: string }[]>({
  ...
  // Sync edit fields when ticket loads
  useEffect(() => {
    if (ticket) {
      setEditFields({
  ...
  const updateField = async (field: string, value: unknown) => {
    ...
    try {
      ...
      console.error('[TicketDetail] updateField failed:', err)
    } finally {
      setUpdating(false)
    }
  }
  ...
  const saveEditField = (field: string) => {
    ...
    if (field === 'sla_due_date') {
      ...
      if (value !== (ticket[field as keyof Ticket] ?? '')) updateField(field, value || null)
    }
  }
  ...
  const submitNote = async () => {
    ...
    if (attachment) {
      ...
      const { data: uploadData, error: uploadErr } = await supabase.storage
        .from('attachments').upload(path, attachment)
            ...
            <div className="p-5 space-y-4">
                  ...
                  {comments.map(comment => (
                    <div key={comment.id} className={`rounded-lg border p-3 text-sm ${
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${comment.is_staff ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300'}`}>
              ...
              <div className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                ...
                <div className="p-3 space-y-2">
                  ...
                  {noteMode === 'reply' && canEmailClient && (
                    <p className="text-xs text-blue-600 dark:text-blue-400">Will be emailed to <strong>{ticket.contact_email}</strong></p>
                  )}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 space-y-4">
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div className="flex-1">
                ...
                <input value={editFields.contact_name} onChange={e => setEditFields(p => ({ ...p, contact_name: e.target.value }))}
                  onBlur={() => saveEditField('contact_name')} placeholder="Contact name" className={inp} />
              </div>
            </div>
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div className="flex-1">
                ...
                <input type="email" value={editFields.contact_email} onChange={e => setEditFields(p => ({ ...p, contact_email: e.target.value }))}
                  onBlur={() => saveEditField('contact_email')} placeholder="email@client.com" className={inp} />
              </div>
            </div>
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div className="flex-1">
                ...
                <input type="datetime-local" value={editFields.sla_due_date}
                  ...
                  onBlur={() => saveEditField('sla_due_date')} className={inp} />
              </div>
            </div>
