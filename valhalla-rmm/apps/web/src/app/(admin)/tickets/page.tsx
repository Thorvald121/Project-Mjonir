...
function getSlaInfo(sla_due_date: string | null | undefined, status: string) {
  if (!sla_due_date || ['resolved','closed'].includes(status)) return { state: 'ok', label: '—' }
  const due = parseISO(sla_due_date)
  if (isPast(due)) return { state: 'breached', label: 'Breached' }
  const h = differenceInHours(due, new Date())
  if (h < 4) return { state: 'warning', label: h < 1 ? '<1h left' : `${h}h left` }
...
function NewTicketDialog({ open, onClose, onSaved, customers, orgId }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!orgId) { setErr('Organization not found. Please refresh and try again.'); return }
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <input value={form.title} onChange={e => s('title', e.target.value)} required
              placeholder="Brief description of the issue" className={`mt-1 ${inp}`} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.contact_name} onChange={e => s('contact_name', e.target.value)}
                placeholder="Jane Smith" className={`mt-1 ${inp}`} />
            </div>
            <div>
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Contact Email</label>
              <input type="email" value={form.contact_email} onChange={e => s('contact_email', e.target.value)}
                placeholder="jane@co.com" className={`mt-1 ${inp}`} />
            </div>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Assign To</label>
            <input value={form.assigned_to} onChange={e => s('assigned_to', e.target.value)}
              placeholder="tech@company.com" className={`mt-1 ${inp}`} />
          </div>
          <div>
          ...
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">
              Tags <span className="normal-case font-normal text-slate-400">(comma-separated)</span>
            </label>
            <input value={form.tags} onChange={e => s('tags', e.target.value)}
              placeholder="vpn, outlook, urgent" className={`mt-1 ${inp}`} />
          </div>
...
export default function TicketsPage() {
  ...
  // Load current user + org on mount
  useEffect(() => {
    const init = async () => {
  ...
  const { data: tickets = [], isLoading, isError } = useQuery<any[]>({
    queryKey: ['tickets'],
    queryFn: async () => {
  ...
  const uniqueCustomers = useMemo(() =>
    [...new Set(tickets.map((t: any) => t.customer_name).filter(Boolean))].sort() as string[]
  , [tickets])

  const uniqueAssignees = useMemo(() =>
    [...new Set(tickets.map((t: any) => t.assigned_to).filter(Boolean))].sort() as string[]
  , [tickets])
  ...
  const filtered = useMemo(() => tickets.filter((t: any) => {
    ...
    if (!['all','mine','unassigned'].includes(assigneeFilter) && t.assigned_to !== assigneeFilter) return false
    return true
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tickets'] }); setSelected(new Set()) },
  })
    ...
    mutationFn: async (status: string) => {
      for (const id of [...selected]) {
        ...
        if (status === 'in_progress' && ticket?.status !== 'in_progress') {
          updates.timer_started = new Date().toISOString()
        } else if (status !== 'in_progress' && ticket?.status === 'in_progress' && ticket?.timer_started) {
          updates.timer_started = null
          const mins = Math.round((Date.now() - new Date(ticket.timer_started).getTime()) / 60000)
          if (mins > 0 && orgId) {
  ...
  const toggleAll   = () => setSelected(prev =>
    prev.size === filtered.length ? new Set() : new Set(filtered.map((t: any) => t.id))
  )
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 rounded-lg">
          ...
          <div className="flex items-center gap-2 ml-2">
            ...
            <button disabled={!bulkStatus || bulkMutation.isPending}
              ...
              className="h-7 px-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded text-xs font-medium transition-colors">
              Apply
            </button>
          </div>
          ...
          <button onClick={() => setSelected(new Set())}
            className="h-7 w-7 flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {isError ? (
                <tr><td colSpan={9} className="px-4 py-12 text-center text-rose-500 text-sm">
                  Failed to load tickets. Check your connection and refresh.
                </td></tr>
              ) : isLoading ? (
                ...
                const sla = getSlaInfo(t.sla_due_date, t.status)
                return (
                  <tr key={t.id}
                    className={`hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors ${selected.has(t.id) ? 'bg-blue-50/50 dark:bg-blue-950/20' : ''}`}>
                    <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                      <input type="checkbox" checked={selected.has(t.id)}
                        onChange={() => toggleSelect(t.id)}
                        className="w-4 h-4 accent-amber-500" />
                    </td>
                    <td className="px-3 py-3 cursor-pointer max-w-xs"
                    ...
                    <td className="px-3 py-3">
                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${PRIORITY_CLS[t.priority] ?? ''}`}>
                        {t.priority}
                      </span>
                    </td>
                    <td className="px-3 py-3">
                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${STATUS_CLS[t.status] ?? ''}`}>
                        {lbl(t.status)}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-3">
                      {t.sla_due_date ? (
                        <div className={`flex items-center gap-1 text-xs font-medium whitespace-nowrap ${
                          sla.state === 'breached' ? 'text-rose-600' : sla.state === 'warning' ? 'text-amber-600' : 'text-slate-400'
                        }`}>
                          {sla.state === 'breached' ? <AlertTriangle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                          {sla.label}
                        </div>
                      ) : <span className="text-slate-300 text-xs">—</span>}
                    </td>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h2 className="font-semibold text-slate-900 dark:text-white text-lg mb-2">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { deleteMutation.mutate(selected); setConfirmDelete(false) }}
