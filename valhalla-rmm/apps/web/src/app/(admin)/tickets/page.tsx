...
import { formatDate, getSlaState, getSlaLabel } from '@valhalla/utils'
...
export default function TicketsPage() {
  ...
  const uniqueCustomers = useMemo(() => [...new Set(tickets.map(t => t.customer_name).filter(Boolean))].sort() as string[], [tickets])
  const uniqueAssignees = useMemo(() => [...new Set(tickets.map(t => t.assigned_to).filter(Boolean))].sort() as string[], [tickets])
        ...
        if (status === 'in_progress' && ticket?.status !== 'in_progress') {
          ...
          if (mins > 0) {
            await supabase.from('time_entries').insert({
              ticket_id: id, ticket_title: ticket.title,
              customer_id: ticket.customer_id, customer_name: ticket.customer_name,
              description: `Time on: ${ticket.title}`, minutes: mins, billable: true,
              hourly_rate: 0, date: new Date().toISOString().split('T')[0],
            })
          }
        }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <button onClick={() => setDialogOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-sm font-semibold transition-colors flex-shrink-0">
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 rounded-lg">
          ...
          <div className="flex items-center gap-2 ml-2">
            ...
            <button disabled={!bulkStatus || bulkMutation.isPending} onClick={() => bulkMutation.mutate(bulkStatus)}
              className="h-7 px-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded text-xs font-medium">Apply</button>
          </div>
      ...
      <NewTicketDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSaved={() => qc.invalidateQueries({ queryKey: ['tickets'] })} customers={customers} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { deleteMutation.mutate(selected); setConfirmDelete(false) }} className="flex-1 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-sm font-semibold">
