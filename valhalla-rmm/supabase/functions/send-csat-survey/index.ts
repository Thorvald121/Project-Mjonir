...
serve(async (req) => {
  try {
    ...
    // Only fire when status changes TO resolved (not closed)
    if (ticket?.status !== 'resolved')          return ok('skipped: not resolved')
...
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#f8fafc;">
  <div style="background:#0f172a;padding:20px 24px;border-radius:12px 12px 0 0;">
    <h2 style="color:#f59e0b;margin:0;font-size:16px;">${orgName}</h2>
  ...
  <div style="background:#ffffff;padding:24px;border-radius:0 0 12px 12px;border:1px solid #e2e8f0;border-top:none;">
    ...
    <p style="color:#475569;font-size:14px;line-height:1.6;">
      Your support ticket <strong style="color:#1e293b;">${ticket.title}</strong> has been resolved.
