...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch customer — uses contact_email/contact_name on customers table
    ...
    if (!customer?.contact_email) {
      return new Response(JSON.stringify({ skipped: true, reason: 'No contact_email on file' }), { status: 200 })
    }
